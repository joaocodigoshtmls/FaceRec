  ...
  return (
    <div className="relative" style={style}>
      ...
      <div className="relative bg-slate-900 rounded-xl overflow-hidden shadow-inner min-h-[400px] flex items-center justify-center">
        ...
        {hasError && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-slate-950/80 backdrop-blur-sm text-white px-6 text-center">
            <p className="text-sm font-semibold">Conexão instável detectada</p>
            <p className="text-xs text-slate-200">
              Tentamos manter o stream em tempo real, mas alternamos para modo snapshot automático.
            </p>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={handleRetry}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/90 text-slate-900 text-xs font-semibold shadow"
            >
              <RefreshCw className="size-4" />
              Tentar stream novamente
            </motion.button>
          </div>
        )}
