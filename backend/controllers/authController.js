...
const resolvedJwtSecret = process.env.JWT_SECRET || process.env.JWT_SECRET_FALLBACK || 'face-rec-dev-secret';

if (!process.env.JWT_SECRET) {
  console.warn('[authController] JWT_SECRET não definido. Usando segredo padrão "face-rec-dev-secret" – defina JWT_SECRET em produção.');
...
export const register = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ sub: userId, id: userId, role: normalizeRole(req.body?.role) || 'professor' }, JWT_SECRET, { expiresIn: '24h' });
  ...
  res.status(500).json({ error: 'Erro no registro', code: 'REGISTER_FAILED' });
  }
};
...
export const login = async (req, res) => {
  ...
  const token = jwt.sign({ sub: DEFAULT_ADMIN_ID, id: DEFAULT_ADMIN_ID, role: 'supervisor' }, JWT_SECRET, { expiresIn: '1h' });
    return res.json({
      token,
      user: {
        id: DEFAULT_ADMIN_ID,
...
export const firebaseLogin = async (req, res) => {
  ...
  const token = jwt.sign({ sub: user.id.toString(), id: user.id.toString(), role: 'professor' }, JWT_SECRET, { expiresIn: '1h' });
